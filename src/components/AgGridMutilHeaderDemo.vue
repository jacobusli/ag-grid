...
// Global error data storage: Map<"rowNo_fieldName", errorArray>
const errorDataMap = new Map<string, Array<{ id: string }>>();

// Helper function to get error key: rowNo_fieldName
const getErrorKey = (rowNo: number, fieldName: string): string => {
  return `${rowNo}_${fieldName}`;
};

// Default column definition with tooltip for all cells
const defaultColDef = ref<ColDef>({
  tooltipValueGetter: (params: any) => {
    // Check if current column has error data in global map
    if (params.colDef.field && params.data.no) {
      const errorKey = getErrorKey(params.data.no, params.colDef.field);
      const errValue = errorDataMap.get(errorKey);
...
for (let i = 1; i <= 50000; i++) {
  ...
  // Store error data in global map (key: "rowNo_fieldName")
  for (const key in data) {
    if (key !== 'no') {
      const errorKey = getErrorKey(i, key);
      errorDataMap.set(errorKey, [{ id: "NB038" }]);
    }
  }
